...
<div id="posts-container" class="fusion-blog-archive facetwp-template <?php echo esc_attr( $wrapper_class ); ?>fusion-clearfix">
    ...
    <?php while ( have_posts() ) : the_post(); ?>
        <?php
            ...
            $link = get_permalink();
            $staff_image = $staffFields['profile_image'];
            ...
            /* Contact Tab */
            $mainPracticeLocation = $staffFields['main_practice_location'];
            $otherPracticeLocations = $staffFields['other_locations'];
        ?>
        <?php
            ...
            <?php if ( ( ( is_search() && Avada()->settings->get( 'search_featured_images' ) ) || ( ! is_search() && Avada()->settings->get( 'featured_images' ) ) ) && 'large-alternate' !== $blog_layout ) : ?>
               	<?php
                    ...
                    elseif ( 'grid' === $blog_layout ) {
                        if ($staff_image) {
                            echo  '<div class="fusion-image-wrapper"><img src="'. $staff_image['url'] .'" alt="'. $staff_image['alt'] .'"/></div>';
                        } else {
                ...
                <h2 class="entry-title fusion-post-title" data-fontsize="26" data-lineheight="31" style="white-space: no-wrap;"> 
                    <?php 
                        ...
                        if ($professionalTitles) {
                            echo '<a href="'. $link .'" style="white-space: no-wrap;">'. $staffFullName .", ";
                            $i = 1;
                            ...
                            echo '<a href="'. $link .'" style="white-space: no-wrap;">'. $staffFullName .'</a>';
                        }
                    ?>
                </h2>
